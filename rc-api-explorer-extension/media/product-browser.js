...
function renderBundleModal(p) {
  ...
  groups.forEach((g, gi) => {
    ...
    div.innerHTML = `<div class="pcg-header">
        ...
        <span class="pcg-meta" id="pcg-meta-${gi}">${esc(metaParts.join(', '))}</span>
      </div>`;
    (g.components || []).forEach((c, ci) => {
      ...
      row.innerHTML = `
        <input type="checkbox" data-gi="${gi}" data-cid="${esc(c.id)}" ${checked?'checked':''} ${isReq?'disabled title="Required"':''}
          onchange="onBmCompToggle('${esc(c.id)}',${gi},this)">
    ...
    errDiv.id = `pcg-err-${gi}`;
    div.appendChild(errDiv);
    body.appendChild(div);
  });
}

window.onBmCompToggle = function(cId, gi, checkbox) {
  ...
  if (checkbox.checked) {
    ...
    if (cnt >= max) {
      ...
      const errEl = document.getElementById(`pcg-err-${gi}`);
...
window.onSaveConfiguration = function() {
  const quoteId = document.getElementById('bm-quote-id').value.trim();
  const pbeId   = document.getElementById('bm-pbe-id').value.trim();
  if (!pbeId) { log('Bundle PricebookEntry ID is required', 'warn'); return; }
...
function onCfgModalStep(msg) {
  ...
  if (ok) {
    ...
    try {
      ...
      // PST returns an array: [{isSuccess, salesTransactionId, errorResponse}]
    ...
    showResult('Bundle creation failed', msg.body);
  }
}
